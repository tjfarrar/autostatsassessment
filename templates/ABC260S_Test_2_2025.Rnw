...
\title{\textbf{\Sexpr{subjectname} (\Sexpr{subjectcode}) \Sexpr{testname} \Sexpr{testno} \ifmemo\textcolor{red}{Memo \the\year}\fi}}
\date{}
\maketitle
...
\begin{table}[h!]
	\begin{tabular}{|l|p{6cm}|p{2.5cm}|p{3.5cm}|}
		\hline
	Qualification Name & \Sexpr{params$qualname} & Qualification Code & \Sexpr{params$qualcode} \\
	\hline
	Examiner & \Sexpr{params$examiner} & Date & \Sexpr{format(params$testdate, "%e %B %Y")} \\
	\hline
	Examiner's Department & Mathematics \& Physics & Time & \Sexpr{params$starttime} -- \Sexpr{params$endtime}\\
	\hline
	Internal Moderator & \Sexpr{params$intmoderator} & Duration & \Sexpr{str_to_title(words(duration))} Hours \\
	\hline
	Total Pages & \pageref{LastPage} & Total Marks & \Sexpr{sum(unlist(qmarks))} \\
	\hline
...
\item \textit{[\Sexpr{sum(qmarks[[1]])} marks]} Answer the following multiple choice questions (two marks each).
...
@
  ...
  \item \textit{[\Sexpr{qmarks[[1]][1]} marks]} What is the capital city of \Sexpr{thecountry}?

  \beq
    \item \Sexpr{thecapital} \ifmemo\textcolor{red}{Correct \tick \tick}\fi
   \item \Sexpr{othercapitals[1]}
  \item \Sexpr{othercapitals[2]}
  \item \Sexpr{othercapitals[3]}
  \eeq
...
@
  ...
  \item \textit{[\Sexpr{qmarks[[1]][2]} marks]} What is the deepest lake in the world?

  \beq
  \item \Sexpr{randanswers[1]} \ifmemo\textcolor{red}{\Sexpr{memotext[1]}}\fi
    \item \Sexpr{randanswers[2]} \ifmemo\textcolor{red}{\Sexpr{memotext[2]}}\fi
  \item \Sexpr{randanswers[3]} \ifmemo\textcolor{red}{\Sexpr{memotext[3]}}\fi 
   \item \Sexpr{randanswers[4]} \ifmemo\textcolor{red}{\Sexpr{memotext[4]}}\fi
  \eeq
...
@
  ...
  \item \textit{[\Sexpr{qmarks[[1]][3]} marks]} What is $\Sexpr{numstoadd[1]}+\Sexpr{numstoadd[2]}$?
...
@
  ...
  \item \textit{[\Sexpr{qmarks[[1]][4]} marks]} Which of the below capital cities is furthest south?

  \beq
  \item \Sexpr{answers[1]} \ifmemo\textcolor{red}{\Sexpr{memotext[1]}}\fi
    \item \Sexpr{answers[2]} \ifmemo\textcolor{red}{\Sexpr{memotext[2]}}\fi
  \item \Sexpr{answers[3]} \ifmemo\textcolor{red}{\Sexpr{memotext[3]}}\fi 
   \item \Sexpr{answers[4]} \ifmemo\textcolor{red}{\Sexpr{memotext[4]}}\fi
  \eeq
...
\item \textit{[\Sexpr{sum(qmarks[[2]])} marks]} Below are given the final marks of a class of \Sexpr{words(n)} students.

\begin{table}[h]
 {\huge\begin{tabular}{\Sexpr{paste(rep("c", n), collapse = "")}}
  \Sexpr{paste(marks, collapse = " & ")}\\
 \end{tabular}}
\end{table}

\beq

  \item \textit{[\Sexpr{qmarks[[2]][1]} marks]} Find the mean of the final marks.

\ifmemo
\textcolor{red}{\begin{align*}
  \bar{x} &= \dfrac{1}{n}\displaystyle\sum_{i=1}^{n} x_i \text{ \tick} \\
  &= \dfrac{1}{\Sexpr{n}}\left[\Sexpr{paste(marks, collapse = " + ")}\right] \text{ \tick} \\
  &= \dfrac{\Sexpr{sum(marks)}}{\Sexpr{n}} = \Sexpr{round2(mean(marks), 4)} \text{ \tick}
\end{align*}}
\fi


  \item \textit{[\Sexpr{qmarks[[2]][2]} marks]} Find the median of the final marks.  
...
\ifmemo
 ...
 \begin{tabular}{\Sexpr{paste(rep("c", n), collapse = "")}}
  \Sexpr{paste(sort(marks), collapse = " & ")}\\
 \end{tabular} 
\end{table}
 \tick \newline
 \Sexpr{methodtext} \tick. Hence, the median is \Sexpr{median(marks)} \tick}
\fi

  \item \textit{[\Sexpr{qmarks[[2]][3]} marks]} Find the standard deviation of the final marks.  

\ifmemo
\textcolor{red}{\begin{align*}
  s^2 &= \dfrac{1}{n-1}\displaystyle\sum_{i=1}^{n} (x_i-\bar{x})^2 \text{ \tick} \\
  &= \dfrac{1}{\Sexpr{n-1}}\left[\left(\Sexpr{marks[1]}-\frac{\Sexpr{sum(marks)}}{\Sexpr{n}}\right)^2 + \left(\Sexpr{marks[2]}-\frac{\Sexpr{sum(marks)}}{\Sexpr{n}}\right)^2 + \cdots + \left(\Sexpr{marks[n]}-\frac{\Sexpr{sum(marks)}}{\Sexpr{n}}\right)^2\right] \text{ \tick} \\
  s &= \sqrt{\Sexpr{var(marks)}} \\
  &= \Sexpr{round2(sd(marks), 4)} \text{ \tick}
\end{align*}}
\fi

  \item \textit{[\Sexpr{qmarks[[2]][4]} marks]} Draw a histogram of the final marks. Use a bin width of 10.
  
<<hist, include=params$domemo>>=
...
@
\item \textit{[\Sexpr{sum(qmarks[[3]])} marks]} Consider the random variable $X$ with probability density function as follows:

  \[ f_X(x) = \begin{dcases*} k(\Sexpr{flatexnok})  & for $\Sexpr{supportlatex}$ \\ 0 & otherwise  \end{dcases*} \]
  
\beq
  \item \textit{[\Sexpr{qmarks[[3]][1]} marks]} Find the value of $k$.

<<findk, echo=FALSE, eval=params$domemo, results='asis'>>=
...
<<findprob, echo=FALSE, eval=TRUE, include=FALSE>>=
set.seed(6543)
...
@
   ...
   \item \textit{[\Sexpr{qmarks[[3]][2]} marks]} Find \(\mathrm{Pr}(X \Sexpr{ineq} \Sexpr{probval})\).

<<echo=FALSE, eval=params$domemo, results='asis'>>=
...
@
  ...
  \item \textit{[\Sexpr{qmarks[[3]][3]} marks]}  Show the probability just calculated on a graph of \(f_X(x)\).
...
\item \textit{[\Sexpr{sum(qmarks[[4]])} marks]} Consider the dataset provided in the Excel file \texttt{surf4cars\_Toyota\_Corolla.xlsx}. A description of the columns in the dataset is as follows. Answer the following questions using R code and output only.

<<echo=FALSE, eval=!params$domemo>>=
...
<<setup_lm_question, eval=TRUE, include=FALSE>>=
set.seed(76)
...
@
  ...
  \item \textit{[\Sexpr{qmarks[[4]][1]} marks]} Fit a linear regression model to the data where Asking Price is the response variable and \Sexpr{explanat[1]} and \Sexpr{explanat[2]} are explanatory variables. Display a summary of the model output. 
...
\fi
  ...
  \item \textit{[\Sexpr{qmarks[[4]][2]} marks]} Predict the asking price of a Toyota Corolla with a \Sexpr{explanat_label[1]} value of \Sexpr{predvalue[1]} and a \Sexpr{explanat_label[2]} value of \Sexpr{predvalue[2]}. Include \Sexpr{100 * conf_level}\% prediction limits.
