...
sympy2latex <- function(s, bracfrac = FALSE, double = TRUE) {
  ...
  if (bracfrac) {
      ...
      for (i in 1:length(s3[[j]])) {
        if (regexpr("/", s3[[j]][i]) == -1) {
          ...
          if (regexpr("[[:alpha:]]", s3[[j]][i]) != -1) {
            ...
            if (double) {
              segs[[j]][i] <- paste0("\\\\frac{", numer, "}{", denom, "}", varpart)
            } else {
              segs[[j]][i] <- paste0("\\frac{", numer, "}{", denom, "}", varpart)
            }
            ...
            if (double) {
              segs[[j]][i] <- paste0("\\\\frac{", numer, "}{", denom, "}")
            } else {
              segs[[j]][i] <- paste0("\\frac{", numer, "}{", denom, "}")
            }
          }
    ...
    if (double) {
      paste0("\\\\frac{", bothparts[1], "}{", bothparts[2], "}")
    } else {
      paste0("\\frac{", bothparts[1], "}{", bothparts[2], "}")
    }
  } else {
    ...
    for (i in 1:length(s3[[1]])) {
      if (regexpr("/", s3[[1]][i]) == -1) {
        ...
        if (regexpr("[[:alpha:]]", s3[[1]][i]) != -1) {
          ...
          if (double) {
            segs[i] <- paste0("\\\\frac{", numer, "}{", denom, "}", varpart)
          } else {
            segs[i] <- paste0("\\frac{", numer, "}{", denom, "}", varpart)
          }
        } else {
          if (double) {
            segs[i] <- paste0("\\\\frac{", numer, "}{", denom, "}")
          } else {
            segs[i] <- paste0("\\frac{", numer, "}{", denom, "}")
          }
        }
...
frac2rational <- function(expr) {
  ...
  if (slashes[1] == -1) {
    ...
    if (exponentpos[1] != -1) {
      # What character precedes each exponent?
    ...
    numb4slash <- vapply(slashes, function(s) 
      regexpr("[[:digit:]]", 
        substr(expr, start = s - 1, stop = s - 1)) != -1, TRUE)
    ...
    if (length(isnumericfraction) == 0) {
      ...
      for (k in seq_along(isnumericfraction)) {
        newexpr <- gsub(pattern = paste0("(?<!\\*\\*)", fractionexpr[k]), 
            replacement = paste0("Rational(", numerator[k], ", ", 
                    denominator[k], ")"), 
              x = newexpr, perl = TRUE)
      }
      newexpr
