...
# The Rnw source file is assumed to have the filename
# subjectcode_testname_testno_currentyear.Rnw

ee <- new.env(parent = globalenv())

# The Rnw file will be rendered twice: the first time to generate the 
#  question paper; the second time to generate the memorandum

invisible(lapply(1:2, function(i) {
  ee$params <- list(title = filenamevec[i], 
                    testdate = testdate, 
                    starttime = starttime,
                    endtime = endtime,
                    ...
                    domemo = memovec[i])
  knit2pdf(input = str_c(str_c(subjectcode, testname, testno, theyear, 
                               sep = "_"), ".Rnw"),
           output = filenamevec[i], clean = FALSE, 
           envir = ee)
}))