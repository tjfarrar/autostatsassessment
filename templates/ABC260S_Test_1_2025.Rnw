...
\beq
  ...
  \item \textit{[\Sexpr{qmarks[[1]][1]} marks]} What is the capital city of Morocco?
  ...
  \item \textit{[\Sexpr{qmarks[[1]][3]} marks]} What is $3+8$?
  ...
  \item \textit{[\Sexpr{qmarks[[1]][4]} marks]} Which of the below countries has the largest population?
  ...
  \item \textit{[\Sexpr{qmarks[[1]][5]} marks]} The most viewed YouTube video of all time is...
...
\item \textit{[\Sexpr{sum(qmarks[[2]])} marks]} Below are given the final marks of a class of twelve students.
...
\fi
  ...
  \item \textit{[\Sexpr{qmarks[[3]][2]} marks]} Find \(\mathrm{Pr}(X > 2)\).
...
\beq
  \item \textit{[\Sexpr{qmarks[[4]][1]} marks]} Fit a linear regression model to the data where Asking Price is the response variable and Year Model, Mileage, and Transmission Type are explanatory variables. Display a summary of the model output. 

<<echo=params$domemo, eval=params$domemo>>=
toyota <- read_excel("../misc/surf4cars_Toyota_Corolla.xlsx")
...
\fi
  ...
  \item \textit{[\Sexpr{qmarks[[4]][2]} marks]} Predict the asking price of a Toyota Corolla with 2018 year model, 100 000 km driven, and a manual transmission. Include 95\% prediction limits.
